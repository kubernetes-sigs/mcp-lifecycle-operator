...
on every cluster that ever stored `v1alpha1` objects. The two steps below must run
...
to be pruned before the served version is removed. Once the migration reports
`Succeeded`:
...
2. Drop `v1alpha1` as a served version **in code**, not with a live `kubectl edit
...
3. Confirm only `v1beta1` remains served and stored:
